...
`PrinterRunnable` implementiert wird.
+
Führen Sie dazu eine Klasse `PrinterRunnable` ein, die das Interface `PrinterRunnable`
implementiert. +
...
Die Verifizierung, ob es sich um eine Primzahl handelt, ist in die Klasse `PrimeTask` ausgelagert, welche bereits `PrinterRunnable` implementiert.
...
Im Moment wird das Resultat nur auf der Konsole ausgegeben, da `PrinterRunnable` kein Resultat zurückgeben können.
...
* **`MandelbrotExecutorProcessor`**: Hier soll ein `ExecutorService` für das Management der Threads verwendet werden. `MandelbrotTask` soll als `PrinterRunnable` implementiert werden, das genau eine Zeile berechnet und diese dem GUI zur Ausgabe übergibt (`processorListener.rowProcessed(row)`). Es müssen also so viele Tasks erzeugt werden, wie das Fenster Zeilen hat (`height`). +
