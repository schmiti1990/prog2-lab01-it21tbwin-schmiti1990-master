...
public class MandelbrotCallableProcessor extends MandelbrotProcessor {
    ...
    private List<Future<ImageRow>> mandelbrotFutures = new ArrayList();
    ...
    public void startProcessing(int numThreads) {
        ...
        for (int i=0; i<height; i++){
            mandelbrotFutures.add(executorService.submit(new MandelbrotTask(i)));
        ...
        try {
            for (Future<ImageRow> row : mandelbrotFutures) {
                processorListener.rowProcessed(row.get());
            }
    ...
    private class MandelbrotTask implements Callable<ImageRow> {
        ...
        public ImageRow call()  {
            ImageRow imageRow = calculateRow(startRow);
